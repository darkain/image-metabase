<?php
  ...
  *                                                       *
  ...
  $page_name = $db_local[$language]['unlinkedtags'];

  require('includes/header.php');


  $db_query = "SELECT m.meta_data, m.meta_id, t.metatype_name, t.metatype_id FROM `$db_meta` m, `$db_metatype` t WHERE m.meta_id NOT IN (SELECT meta_id FROM `$db_metalink` UNION SELECT meta_child FROM `$db_metalink`) AND m.metatype_id=t.metatype_id ORDER BY t.metatype_name ASC, m.meta_data ASC";
  $db_result = mysql_query($db_query);
  $db_count  = mysql_numrows($db_result);

  echo "<h1>" . $db_local[$language]['unlinkedtags'] . "</h1><div>";
